<x-admin-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-serif text-2xl font-semibold text-coffee-900">Edit Menu</h2>
            <p class="mt-1 text-sm text-coffee-500">Ubah detail menu; upload foto baru untuk menggantikan yang lama.</p>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('admin.menu.update', $menu) }}" enctype="multipart/form-data" class="card max-w-3xl p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $menu->name) }}" placeholder="Contoh: Cappuccino"
            ...
            <div>
                ...
                <input id="price" name="price" type="number" step="100" min="0" value="{{ old('price', $menu->price) }}" placeholder="18000"
                    class="w-full rounded-cozy border-coffee-100 bg-white px-3.5 py-2.5 text-sm text-coffee-800 placeholder:text-coffee-400 focus:border-terracotta-500 focus:ring-terracotta-500" />
            ...
            <div>
                ...
                <div x-data="{ preview: null }" class="flex items-start gap-3">
                    ...
                    <div class="shrink-0">
                        @if ($menu->image_url)
                            <img src="{{ $menu->image_url }}" alt="{{ $menu->name }}" class="h-20 w-20 rounded object-cover shadow-warm" />
                        @endif
                        <template x-if="preview">
                            <img :src="preview" alt="Pratinjau foto baru" class="mt-2 h-20 w-20 rounded object-cover shadow-warm" />
                        </template>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-5">
            ...
            <textarea id="description" name="description" rows="3" placeholder="Kisahkan bahan & cita rasa menu ini..."
                class="w-full rounded-cozy border-coffee-100 bg-white px-3.5 py-2.5 text-sm text-coffee-800 placeholder:text-coffee-400 focus:border-terracotta-500 focus:ring-terracotta-500">{{ old('description', $menu->description) }}</textarea>
            @error('description')<p class="mt-1.5 text-xs font-medium text-terracotta-500">{{ $message }}</p>@enderror
        </div>

        <div class="mt-5 flex items-center gap-3">
            <input id="is_available" name="is_available" type="checkbox" value="1" @checked(old('is_available', $menu->is_available))
