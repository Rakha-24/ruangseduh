<x-admin-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-serif text-2xl font-semibold text-coffee-900">Tambah Promo</h2>
            <p class="mt-1 text-sm text-coffee-500">Tentukan judul, periode berlangsung, dan banner promonya.</p>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('admin.promo.store') }}" enctype="multipart/form-data" class="card max-w-3xl p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                ...
                <input id="title" name="title" type="text" value="{{ old('title') }}" placeholder="Contoh: Happy Hour 16.00 - 18.00"
            ...
            <div>
                ...
                <div x-data="{ preview: null }">
                    ...
                    <template x-if="preview">
                        <img :src="preview" alt="Pratinjau banner promo" class="mt-3 h-28 w-40 rounded object-cover shadow-warm" />
                    </template>
                </div>
