<x-admin-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-serif text-2xl font-semibold text-coffee-900">Edit Meja</h2>
            <p class="mt-1 text-sm text-coffee-500">Perbarui info meja "{{ $diningTable->name }}".</p>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('admin.dining-tables.update', $diningTable) }}" class="card max-w-2xl p-6">
        @csrf
        @method('PATCH')
        ...
        <div>
            ...
            <input
                ...
                name="name"
                type="text"
                value="{{ old('name', $diningTable->name) }}"
        ...
        <div class="mt-6">
            ...
            <input
                ...
                min="1"
                max="500"
                value="{{ old('capacity', $diningTable->capacity) }}"
                class="w-full rounded-cozy border-coffee-100 bg-white px-3.5 py-2.5 text-sm text-coffee-800 placeholder:text-coffee-400 focus:border-terracotta-500 focus:ring-terracotta-500"
            />
            @error('capacity')
                <p class="mt-1.5 text-xs font-medium text-terracotta-500">{{ $message }}</p>
        ...
        <div class="mt-6">
            <label class="flex items-center gap-3 rounded-cozy border border-coffee-100 bg-cream-50 px-4 py-3">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $diningTable->is_active)) class="h-5 w-5 rounded border-coffee-200 text-terracotta-500 focus:ring-terracotta-500">
                <span>
                    <span class="block text-sm font-semibold text-coffee-800">Meja aktif</span>
                    <span class="block text-xs text-coffee-500">Nonaktifkan meja yang sedang rusak agar tidak menerima reservasi baru.</span>
                </span>
            </label>
        </div>
