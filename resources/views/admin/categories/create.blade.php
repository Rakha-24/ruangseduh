<x-admin-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-serif text-2xl font-semibold text-coffee-900">Tambah Kategori</h2>
            <p class="mt-1 text-sm text-coffee-500">Slug dihasilkan otomatis dari nama (untuk URL yang ramah SEO).</p>
        </div>
    </x-slot>

    <form method="POST" action="{{ route('admin.categories.store') }}" class="card max-w-2xl p-6">
        @csrf
